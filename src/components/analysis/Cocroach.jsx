...
const Cocroach = () => {
  ...
  const rows = 10;
  const cols = 23;
  const [displayedResult, setDisplayResult] = useState(Array.from({ length: rows }, () => Array.from({ length: cols })));
  ...
  return (
    <>
      <div className={`d-flex  gap-1  `}>
        <div className={`   d-flex justify-content-center   ${s.main}`} style={{ width: '70vw' }}>
              ...
              <div className={`d-flex gap-1 `} key={colIndex}>
                {column.map((row, rowIndex) => (
                  <div
                    ...
                    <div
                      className={`rounded-circle ${row === 'A' || row === 'B' ? 'shadow-xs' : ''}`}
                      style={{
                        height: '1.7vw',
                        width: '1.7vw',
                        backgroundColor: row === 'A' ? 'rgb(4, 122, 45)' : row === 'B' ? 'rgb(194, 11, 11)' : 'transparent'
                      }}></div>{' '}
                  </div>
                ))}
              </div>
