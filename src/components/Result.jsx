...
const Result = ({ clearSetCard, children, card /*  joker, andarCards, baharCardss */ }) => {
            ...
            <div style={{ width: '90%' }} className="d-flex   h-50 ps-4 pe-5 ">
              <div className=" d-flex   justify-content-center align-items-center">
                <div className="  d-flex flex-column justify-content-center align-items-center h-100  text-center fw-bold mx-2 mb-2">
                  ...
                  {joker ? <SlideDownImage image={joker ? joker.card : BackgroundCard} height={'40%'} position={'static'} alt="Andar Card" animationOrder={'down'} /> : <img className={`visually-hiddens`} style={{ height: '40%' }} src={BackgroundCard} />}
                </div>
              </div>
              <div className="d-flex  h-100 flex-column  justify-content-center w-100" style={{ paddingTop: '1.3rem', paddingBottom: '1.3rem' }}>
                <div className=" px-2  h-50 w-100 " style={{ position: 'relative' }}>
                  <div className=" border-bottom fs-3  text-center mb-1 fw-bold">andar</div>
                  {andarCards.length > 0 ? andarCards.map((card, i) => <SlideDownImage image={card?.card} i={i + 3} alt="Andar Card" />) : <img className={`visually-hiddens`} style={{ height: '80%' }} src={BackgroundCard} />}
                </div>

                <div className=" h-50    px-2 w-100 " style={{ position: 'relative' }}>
                  <div className="border-bottom   fw-bold text-center fs-3 mb-1">bahar</div>
                  {baharCards.length > 0 ? baharCards.map((card, i) => <SlideDownImage image={card?.card} i={i + 3} alt="Bahar Card" />) : <img className={`visually-hiddens`} style={{ height: '80%' }} src={BackgroundCard} />}
                </div>
              </div>
            </div>
          </div>
